...
class ThesisGenerator:
    ...
    def setup_styles(self):
        ...
        # 一级标题（章）
        self._add_heading_style('Heading1', '章标题', Pt(18), bold=True)

        # 二级标题（节）
        self._add_heading_style('Heading2', '节标题', Pt(16), bold=True)

        # 三级标题（小节）
        self._add_heading_style('Heading3', '小节标题', Pt(14), bold=True)

        # 四级标题（小小节）
        self._add_heading_style('Heading4', '小小节标题', Pt(12), bold=True)

    def _add_heading_style(self, style_id, style_name, font_size, bold=True):
        """添加标题样式"""
        if style_id in self.doc.styles:
            style = self.doc.styles[style_id]
        else:
    ...
    def add_english_abstract(self):
        ...
        This thesis first analyzes the research background and significance of intelligent sports training systems, and reviews the development status and research progress of large language models, retrieval-augmented generation technology, multi-agent systems, and vector databases. On this basis, the overall system architecture design is proposed, including the multi-agent collaborative training support subsystem, advanced retrieval-augmented generation subsystem, and multi-level memory management system.
        ...
        keywords = "Large Language Model; Retrieval-Augmented Generation; Multi-Agent System; Intelligent Sports Training; Vector Database"
        run = p.add_run(keywords)
        run.font.size = Pt(12)
        run.font.name = 'Times New Roman'
        run.font.bold = False
    ...
    def add_table_of_contents(self):
        ...
        for i, (title, page) in enumerate(toc_data, 1):
            ...
            for cell in cells:
                ...
                if title.startswith('第') or title == '摘  要' or title == 'ABSTRACT' or title == '目  录' or title == '参考文献' or title == '致  谢' or title == '附  录':
                    para.runs[0].font.bold = True
    ...
    def add_chapter(self, number, title):
        """添加章节标题"""
        self.doc.add_paragraph().add_run(f"第{number}章  {title}").style = 'Heading1'

    def add_section(self, number, title):
        """添加节标题"""
        self.doc.add_paragraph().add_run(f"{number}  {title}").style = 'Heading2'

    def add_subsection(self, number, title):
        """添加小节标题"""
        self.doc.add_paragraph().add_run(f"{number}  {title}").style = 'Heading3'

    def add_sub_subsection(self, number, title):
        """添加小小节标题"""
        self.doc.add_paragraph().add_run(f"{number}  {title}").style = 'Heading4'
    ...
    def add_paragraph_with_note(self, text, note):
        """添加带注释的段落"""
        p = self.doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        p.paragraph_format.line_spacing = 1.5
    ...
    def add_empty_paragraphs(self, count=1):
        """添加空行"""
        for _ in range(count):
            self.doc.add_paragraph()

    def add_table(self, data, headers, caption=None, caption_num=None):
        """添加表格"""
        if caption:
            # 表格标题
            p = self.doc.add_paragraph()
            ...
            run = p.add_run(f"表{caption_num}  {caption}")
    ...
    def add_figure_caption(self, caption, num):
        """添加图标题"""
        p = self.doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.paragraph_format.space_before = Pt(6)
        p.paragraph_format.space_after = Pt(12)
        run = p.add_run(f"图{num}  {caption}")
        run.font.size = Pt(10.5)
        run.font.name = '宋体'
...
if __name__ == "__main__":
    ...
    # 添加中文摘要
    generator.add_chinese_abstract()

    # 添加英文摘要
    generator.add_english_abstract()

    # 添加目录
    generator.add_table_of_contents()

    # 保存文档
    output_path = "/Users/liubingyan/Sports-Training-Agent/thesis_output/基于多智能体与检索增强生成的智能运动训练系统.docx"
    generator.save(output_path)
    print(f"论文模板生成完成: {output_path}")
